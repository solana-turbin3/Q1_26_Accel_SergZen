...
use crate::{constants::AUTO_REFUND_TIME, errors::EscrowError, state::Escrow};

#[derive(Accounts)]
pub struct RefundAuto<'info> {
    ///CHECK: for crankers to sign it just passing maker
    #[account(
        mut,
...
impl<'info> RefundAuto<'info> {
    pub fn refund_and_close_vault(&mut self) -> Result<()> {
        let clock = Clock::get()?;
        require!(
            clock.unix_timestamp >= (self.escrow.created_at + AUTO_REFUND_TIME),
